...
def initialize_browser():
    ...
    const_adblock_xpi_path = (
        "res/uBlock0.firefox.xpi"  # load Firefox with uBlock Origin enabled
    )
...
def grab_data(browser):
    ...
    for category, xpath in zip(column_header, column_data_source):
        try:
            result_dict[category] = browser.find_element_by_xpath(xpath).text.encode(
                "utf-8"
            )
        except:
